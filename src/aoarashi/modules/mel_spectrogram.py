    ...
    def forward(self, speech: torch.Tensor, length: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            tuple[torch.Tensor, torch.Tensor]:
                torch.Tensor: Mel-spectrogram tensor (batch, sample // hop_size + 1, mel_size).
...
class LogMelSpectrogram(MelSpectrogram):
    def __init__(
        self, fft_size: int, hop_size: int, window_size: int, mel_size: int, sample_rate: int, eps: float = 1e-12
    ):
        super().__init__(fft_size, hop_size, window_size, mel_size, sample_rate)
        self.eps = eps
    ...
    def forward(self, speech: torch.Tensor, length: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            tuple[torch.Tensor, torch.Tensor]:
                ...
                torch.Tensor: Mask tensor (batch, sample // hop_size + 1).
        """
        x, mask = super().forward(speech, length)
