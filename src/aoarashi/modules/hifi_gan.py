...
def generator_adversarial_loss(fake_xs: list[torch.Tensor]) -> torch.Tensor:
    """
    ...
    for fake_x in fake_xs:
        ...
        loss = mse_loss(fake_x, torch.ones_like(fake_x), reduction="mean")
        losses.append(loss)
    loss = torch.stack(losses).sum()
    return loss


def discriminator_adversarial_loss(real_xs: list[torch.Tensor], fake_xs: list[torch.Tensor]) -> torch.Tensor:
    """
    ...
    for real_x, fake_x in zip(real_xs, fake_xs):
        ...
        loss = mse_loss(real_x, torch.ones_like(real_x), reduction="mean") + mse_loss(
            fake_x, torch.zeros_like(fake_x), reduction="mean"
        )
        losses.append(loss)
    loss = torch.stack(losses).sum()
    return loss
...
def feature_matching_loss(real_feats: list[torch.Tensor], fake_feats: list[torch.Tensor]):
    """
    ...
    for real_feat, fake_feat in zip(real_feats, fake_feats):
        assert real_feat.shape == fake_feat.shape
        # NOTE: L1 loss averaged over the batch and feature size
        l1 = l1_loss(real_feat.flatten(), fake_feat.flatten(), reduction="mean")
        losses.append(l1)
    loss = torch.stack(losses).sum()
    return loss


def mel_spectrogram_loss(fake_mel: torch.Tensor, real_mel: torch.Tensor):
    """

    Args:
        fake_mel (torch.Tensor): Mel-spectrogram tensor from the generator (batch, frame, mel_size).
        real_mel (torch.Tensor): Mel-spectrogram tensor from the ground truth audio (batch, frame, mel_size).

    Returns:
        torch.Tensor: Mel-spectrogram loss.
    """
    return l1_loss(fake_mel, real_mel, reduction="mean")
...
class PeriodDiscriminator(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, list[torch.Tensor]]:
        ...
        x = torch.nn.functional.pad(x, (0, self.period * math.ceil(t / self.period) - t), "constant", 0.0)
        x = x.view(-1, 1, math.ceil(t / self.period), self.period)
...
class Generator(nn.Module):
    def __init__(
        ...
        self.reaky_relu = nn.LeakyReLU(negative_slope)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for upsample in self.upsamples:
            x = upsample(x)
        x = self.reaky_relu(x)  # (batch, hidden_size / 2^4, frame)
...
class Discriminator(nn.Module):
    def __init__(self, periods: list[int] = [2, 3, 5, 7, 11], scales: list[int] = [1, 2, 4]):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> tuple[list[torch.Tensor], list[torch.Tensor]]:
        ...
        for msd in self.msd:
            ...
            x_list.append(x_o)
            feats_list.append(feats)
        feats_list = list(chain(*feats_list))
