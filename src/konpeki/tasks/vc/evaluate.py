...
from konpeki.tasks.vc.dataset import Dataset
...
def main(config: DictConfig):
    ...
    for i in tqdm(range(len(test_dataset))):
        ...
        src_wav = sample["src_audio"].to(device)
        tgt_wav = sample["tgt_audio"].cpu().numpy()
        gen_wav = model.synthesize(src_wav.squeeze())  # (1, 1, sample)
        mcd = calculate_mel_cepstral_distortion(
            gen_wav.squeeze().cpu().numpy(),
            tgt_wav,
            config.model.fft_size,
            config.model.hop_size,
